class DES:
    ...
    def prepare_key(self, key: bytes):
        ...
        key_bin = ''.join(format(byte, '08b') for byte in key[:8])  # Limita a 64 bits
        key_permuted = self.permute(key_bin, self.PC1)
        return key_permuted

    def generate_subkeys(self, key_bin):
        """Gera as 16 subchaves para cada rodada."""
        C, D = key_bin[:28], key_bin[28:]
        subkeys = []
        for shift in self.SHIFTS:
            C, D = self.left_shift(C, shift), self.left_shift(D, shift)
            subkeys.append(self.permute(C + D, self.PC2))
        return subkeys
    ...
    def add_padding(self, data: bytes) -> bytes:
        ...
        if padding_length == 0:
            return data
        padding = bytes([padding_length] * padding_length)  # Padding com valor do tamanho
        return data + padding

    def encrypt(self, plaintext: bytes) -> bytes:
        """Criptografa um bloco de 64 bits."""
        # Adiciona padding para garantir que o texto tenha 64 bits
        plaintext = self.add_padding(plaintext)

        # Converte os 64 bits em uma string binária
        plaintext_bin = ''.join(format(byte, '08b') for byte in plaintext)

        # Permutação inicial
        permuted_text = self.permute(plaintext_bin, self.IP)
        L, R = permuted_text[:32], permuted_text[32:]

        for i in range(16):
            new_R = self.xor(L, self.feistel(R, self.subkeys[i]))
            L, R = R, new_R

        cipher_bin = self.permute(R + L, self.IP_INV)
        return self.bin_to_bytes(cipher_bin)

    def decrypt(self, ciphertext: bytes) -> bytes:
        """Descriptografa um bloco de 64 bits."""
        ciphertext_bin = ''.join(format(byte, '08b') for byte in ciphertext)
        permuted_text = self.permute(ciphertext_bin, self.IP)
        L, R = permuted_text[:32], permuted_text[32:]
        ...
        plain_bin = self.permute(R + L, self.IP_INV)
        return self.bin_to_bytes(plain_bin)

    def feistel(self, R, subkey):
        """Função Feistel com expansão, XOR, S-Box e permutação."""
        expanded_R = self.permute(R, self.EXPANSION)
        xored = self.xor(expanded_R, subkey)
    ...
    def bin_to_bytes(self, bin_str):
        """Converte uma string binária para bytes."""
    ...
    def s_box_substitution(self, data):
        ...
        for i in range(8):
            ...
            col = int(block[1:5], 2)
            output += format(self.S_BOX[i][row][col], '04b')
        return output

    # Tabelas usadas pelo DES (não modifiquei as tabelas aqui)
    # ...
